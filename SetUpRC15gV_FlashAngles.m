...
% 前32个阵元 (行) 分布在 X 轴
Trans.ElementPos(1:32, 1) = Trans.spacing * (-15.5:15.5);
% 后32个阵元 (列) 分布在 Y 轴
Trans.ElementPos(33:64, 2) = Trans.spacing * (-15.5:15.5);
...
PData.Origin = [-Trans.spacing*15.5,Trans.spacing*15.5,P.startDepth]; % x,y,z of upper lft crnr of page 1.
...
% 改：以上Media.MP坐标是按80阵元(半孔径39.5)设计的，需按新孔径比例缩放，否则外围点会落在阵列范围外
apertureScale = 15.5/39.5;
Media.MP(:,1) = Media.MP(:,1) * apertureScale;
Media.MP(:,2) = Media.MP(:,2) * apertureScale;
