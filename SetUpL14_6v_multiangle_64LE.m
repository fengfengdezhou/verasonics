...
% Vantage型号：64LE
% 换能器型号：L14-6
% 说明：官方例程修改，接收并显示64通道数据
% 改动：2020.08.21  创建

clear 
P.startDepth = 15;	% Acquisition depth in wavelengths
P.endDepth = 135;   % This should preferrably be a multiple of 128 samples.
...
Resource.VDAS.dmaTimeout = 10000;           % 未知
...
Trans.elementWidth = 0.16;                          % element width in mm; assumes 50 micron kerf
Trans.spacingMm = 0.2;                             % Spacing between elements in mm.
...
PData(1).PDelta = [1, 0, 1] * 0.5;         	% [Trans.spacing, 0, 0.5]
...
Resource.DisplayWindow(1).Title = 'L14-6vmultiAngles_64LE';
...
TX = repmat(struct('waveform', 1, ...
                   'Origin', [0.0,0.0,0.0], ...
                   'Apod', kaiser(Resource.Parameters.numTransmit,1)', ...  %kaiser chuang han shu
                   'focus', 0.0, ...
                   'Steer', [0.0,0.0], ...
...
SeqControl(2).argument = 20000 - (na - 1) * 160;  % 20 msec
...
% Save all the structures to a .mat file.
  ...
  save('MatFiles/L14-6_multiangle_LE');
  filename = ('L14-6_multiangle_LE');
VSX;
return
