...
% Define system parameters.只修改通道
...
% %Specify Trans structure array.探头的信息
% 手动定义 Trans 结构体
Trans.name = 'RC3.5-32x32';      % 自定义探头名称，避免系统检查探头ID
Trans.id = -1;              % 强制忽略探头ID检查
Trans.units = 'mm';         % 单位使用毫米
Trans.frequency = 3.5;      % 中心频率 (MHz)
Trans.Bandwidth = [2.45, 4.55]; % 带宽 (MHz)
Trans.type = 4;             % 类型 4：RC 行列阵列
Trans.connType = 1;         % 物理接口类型 (1 为 Verasonics 260针 HDI 接口)
Trans.numelements = 64;     % 总阵元数 (32行 + 32列)
Trans.spacingMm = 0.495;    % 阵元间距 (mm)
Trans.elementWidth = 0.47; % 阵元物理宽度 (通常略小于间距，设为0.9倍)
Trans.elementLength = 15.84; % [RC阵列专属] 阵元长度 (mm)
Trans.lensCorrection = 0;   % 声透镜补偿 (无)
Trans.impedance = 50;       % 阵元阻抗 (暂无实测，暂定50欧姆)
Trans.maxHighVoltage = 50;  % 最大耐受高压 (±50V)

% 手动生成阵元坐标
Trans.ElementPos = zeros(Trans.numelements, 5);
% 前32个阵元 (行) 分布在 X 轴
Trans.ElementPos(1:32, 1) = Trans.spacingMm * (-15.5:15.5);
% 后32个阵元 (列) 分布在 Y 轴
Trans.ElementPos(33:64, 2) = Trans.spacingMm * (-15.5:15.5);

% 阵元到接头的映射表 (非常重要，通常需要根据实际的物理接线图填写)
% 此处假设为最简单的 1:1 顺序映射 (阵元1接引脚1，阵元2接引脚2...)
Trans.ConnectorES = (1:Trans.numelements)';
% ---- 统一将mm单位换算成波长单位 ----
scaleToWvl = Trans.frequency/(Resource.Parameters.speedOfSound/1000); % mm转波长的比例因子
Trans.spacing = Trans.spacingMm * scaleToWvl;          % 阵元间距(波长)，后面PData等要用
Trans.elementWidth = Trans.elementWidth * scaleToWvl;  % 阵元宽度也换算成波长（如果后面代码里有用到的话）

% Specify PData structure array.
PData.PDelta = [Trans.spacing, Trans.spacing, 0.5];
PData.Size(1) = 32;%行列阵元
PData.Size(2) = 32;
PData.Size(3) = ceil((P.endDepth-P.startDepth)/PData.PDelta(3)); % startDepth, endDepth and pdelta set PData.Size(3).
PData.Origin = [-Trans.spacing*15.5,Trans.spacing*15.5,P.startDepth]; % x,y,z of upper lft crnr of page 1.改为15.5，探头半孔径的阵元数偏移
...
Resource.DisplayWindow(1).Title = 'RC3g';
...
Resource.DisplayWindow(2).Title = 'RC3g';
...
Resource.DisplayWindow(3).Title = 'RC3g';
...
% - Set event specific TX attributes.修改的阵元数量
    TX(1).Apod(1:32) = 1;
    TX(2).Apod(33:64) = 1;
...
maxAcqLength = ceil(sqrt(P.endDepth^2 + (32*Trans.spacing)^2));
...
%     Receive((i-1)+1).callMediaFunc = 1;
    m = 2*(i-1); % m counts frames
        ...
        Receive(m+1).Apod(33:64) = 1; 
        Receive(m+1).framenum = i;
        Receive(m+1).acqNum = 1;
        Receive(m+2).Apod(1:32) = 1;
...
filename = 'RC15gV_Flash';   % 注意：这里只写文件名，不带路径和.mat后缀
