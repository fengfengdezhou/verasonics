...
KnownTransducers = {'L7-4',       '000250',  96,   0;...
                    ...
                    'M3dV',                      '0D0400',  45,   0;... % Vermon Single Panel 1024 element 3 MHz Matrix array
...
%                     Trans.elevationFocusMm = TBD; % nominal elevation focus depth from lens on face of transducer
            ...
            case 'GEL8-18iD'
                %% GEL8-18iD Specification
                if ~isfield(Trans,'frequency'), Trans.frequency = 9.0; end %
                if ~isfield(Trans,'Bandwidth'), Trans.Bandwidth = [4.0, 14.0]; end
                Trans.type = 0;     %linear array
                Trans.connType = 7; % =7 GE connector
                Trans.numelements = 168;
                Trans.spacingMm = 0.1497; % spacing in mm.
                Trans.elementWidth = 0.9 * Trans.spacingMm; % width in mm (guess)
                   Trans.elevationApertureMm = 4.0; % active elevation aperture in mm                   
                   Trans.elevationFocusMm = 13.5;
                % Set default element positions (units in mm).
                Trans.ElementPos = zeros(Trans.numelements,5);
                Trans.ElementPos(:,1) = Trans.spacingMm*(-((Trans.numelements-1)/2):((Trans.numelements-1)/2));
                Trans.lensCorrection = 0.625; % in mm units;
                Trans.impedance = 50;    % (guess)
                Trans.maxHighVoltage = 50; % conservative value, max. voltage not known
                Trans.ConnectorES = (45:212)';
...
%                     4.0, 28.0+35.0i; 4.5, 42.0+83.0i; 5.0, 75.0+125.0i; 5.5, 128.0+159.0i; 6.0, 201.0+178.0i];
            ...
            case 'M3dV'
                %% M3dV   Vermon Single Panel 1024 EL 3 MHz Matrix Array
                if ~isfield(Trans,'frequency'), Trans.frequency = 3.0; end
                if ~isfield(Trans,'Bandwidth'), Trans.Bandwidth = Trans.frequency*[0.75, 1.25]; end % 50 percent bandwidth at -6dB
                Trans.connType = 8;
                ...
                Trans.ElementPos = getVermonMatrixElpos(Trans.spacingMm, true); % ElementPos in mm units. Single panel
                Trans.impedance = [1, 27.69-821.22i;  1.125, 27.03-734.93i;  1.25, 28.52-660.11i;  1.375, 30.14-597.19i;...
                    1.5, 27.19-549.13i;  1.625, 27.48-506.71i;  1.75, 28.65-469.92i;  1.875, 28.48-438.83i;  2, 28.09-412.03i;...
                    2.125, 27.79-387.39i;  2.25, 27.03-365.68i;  2.375, 27.17-346.57i;  2.5, 27.50-328.99i;  2.625, 27.29-313.56i;...
                    2.75, 27.14-298.96i;  2.875, 26.83-286.13i;  3, 26.21-274.24i;  3.125, 26.02-262.98i;  3.25, 26.11-252.60i;...
                    3.375, 25.83-243.08i;  3.5, 25.54-234.13i;  3.625, 25.59-226.13i;  3.75, 25.47-218.63i;  3.875, 25.06-211.13i;...
                    4, 24.78-204.39i;  4.125, 24.59-197.84i;  4.25, 24.15-191.55i;  4.375, 24.14-185.59i;  4.5, 24.06-180.09i;...
                    4.625, 23.96-174.79i;  4.75, 23.80-169.66i;  4.875, 23.86-164.79i;  5, 23.83-160.30i];
            ...
            case 'Matrix1024-3'
                ...
                % Trans.ConnectorES is the overall mapping from probe elements to
                % the 1024 Element Signals as defined by the connType 8 interface.
                Trans.ElementPos = getVermonMatrixElpos(Trans.spacingMm, false); % ElementPos in mm units. 4 panel
...
function ELpos = getVermonMatrixElpos(spacing, singlePanel)
    ...
    for ii = 1:GSRow
        if ii==1; jj=0; end
        if ~singlePanel % add empty rows between panels
            if ii==9 || ii==18 || ii==27
                continue;
            end
        end
