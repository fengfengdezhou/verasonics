%
% File name: SetUpM3dVFlash.m - Example of imaging with single
%                                   plane wave using the Matrix 3MHz probe
%                                  
% Description:
%  Sequence programming file for 3MHz matrix probe for plane wave imaging using.      
%  This script demonstrates use of dynamic mux programming using the 1024 element 
%  3MHz matrix probe using the UTA 1024-MUX adapter on a Vantage NXT 256 system.  
%  The  "Papod" variable is used to specify the desired apertures. Each
%  entry in Papod specified the full 1024 element, where only 256 entries
%  can be ones. The apertures are synthesized by the "computeMuxAperture" 
%  function to match the user-defined Apod for each TX and Receive.
%  The user needs to be aware that in the 1024 MUX UTA and the
%  3MHz matrix probe by Vermon, elements 1, 257, 513 and 769 are connected
%  to the same channel using the Muxing circuity. The same is true for the
%  rest on the system channels. 
%  In this example script, acquisition with all 1024 element acquisition is
