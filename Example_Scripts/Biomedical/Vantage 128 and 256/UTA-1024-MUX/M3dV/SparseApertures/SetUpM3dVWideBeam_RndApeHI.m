%
% File name: SetUpM3dVWideBeam_RndApeHI.m - Example of WideBeam harmonic 
%                       imaging for volume acquisitions using a matrix probe (3 MHz),
%                       Random Aperture in Transmit and synthetic apertures in Receive  
%                                  
% Description:
%  This scripts transmits and receives 100 wide beams focused beyond the
%  imaging field which are then compounded in the volume reconstruction. 
%  Each beam is transmitted with positive and negative polarity and receive
%  signal is accumulated in the hardware. 
%  The scanning patern in this script follows a grid of points at the
...
%  transmits and the Regions used in the reconstruction.  
%  The user should keep in mind that in the Vantage system a maximum of 255 
%  different MUX apertures can be used. 
%  This script uses random apertures for transmit, each with 256
%  elements. The random aperture used in this script are loaded from a file 
%  named "Saved256RndApodOrigin" which provides apertures with a higher density 
%  of elements towards the center of the probe than in the perifery. 
%  are all defined accounting for the multiplexer constraints. 
%  The receive is done with four subapertues (1-256, 257-512, 
%  513-768, 769-1024). Therefore every transmit needs to be repeated four 
%  times so that it can be received with each subapertures.
%  
%  This script can reconstruct either 3 orthogonal planes (XZ, YZ and XY)
%  by setting ReconRegion = 5, or can reconstruct the entire volume (a
%  pyramid shape) using ReconRegion = 1. In the case of the orthogonal
%  reconstruction the script uses the center planes (XZ and YZ) with
%  respect to the probe, while the XY plane can be chosen using the
%  variable XYDisp. 
%
% Notice:
...
save('MatFiles/M3dV-WideBeam-RndApeHI.mat');
